...
class InfCheck:
    ...
    def inf_status(self, inf_name):
        ...
        self.inf_name = subprocess.call(["ifconfig",self.inf_name], stdout=open(os.devnull, 'wb'))
        ...
        if self.inf_name == 1:
            ...
            exit()
        elif self.inf_name == 0:
            #Check if the interface is up"
            self.get_can_inf = "ip a show " + self.inf_name
            process = subprocess.Popen(self.get_can_inf.split(), stdout=subprocess.PIPE)
            output, error = process.communicate()
            self.bus = can.interface.Bus(self.inf_name, bustype='socketcan')
            if "DOWN" in output:
                print("ERROR: The CAN Bus interface is DOWN, please activate it and restart the application.")
                exit()
            else:
                self.bus = can.interface.Bus(self.inf_name, bustype='socketcan')
            return self.bus